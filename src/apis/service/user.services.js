...
module.exports = {
    signup: async (request) => {
        try {
            let { email_id, signup_type, password, post_code, social_login_id, user_type } = request;
            let user, token_type = "";
            ...
            user['address'] = user_type == 2 ? await _repos.createAddress(request) : null
    ...
    login: async (request) => {
        try {
            ...
            let token = await jwtToken({ user_id: getUser.user_id, type: token_type });
    ...
    UserOrderList: async (request, user) => {
        try {
            ...
            let { rows, count } = await _repos.getUserOrder(where, where1, limit, offset);
    ...
    ProductOrdered: async (request, user) => {
        try {
            ...
            let { rows, count } = await _repos.productOrdered(where, where1, limit, offset);
